...
import { fillerStyle, rowStyle } from "./styles";
...
export const ListDisplay = ({list, name}: {list: string[], name: string}) => {
  ...
  return (
    <Paper sx={{...rowStyle, overflow: "hidden", textOverflow: "ellipsis"}}>
      ...
      <Box onClick={onOpenModal} sx={{...fillerStyle, cursor: "pointer", overflow: "hidden"}}>
        <Typography sx={{width: "100%", whiteSpace: "nowrap", textOverflow: "ellipsis", textAlign: "left"}}>
          {`[${list.join(", ")}]`}
        </Typography>
      </Box>
